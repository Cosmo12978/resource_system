...
util.AddNetworkString( "Operationirongate.Convoymenu" )
util.AddNetworkString( "Operationirongate.Convoystart" )
...
net.Receive("Operationirongate.Convoystart", function(len, ply)
	...
	local carData = USCONVOY.Cars[id]
	if not carData then return end

	local pos1 = USCONVOY.Cfg.SpawnBox.minPos
	local pos2 = USCONVOY.Cfg.SpawnBox.maxPos
	...
	if carinbox == false then
		...
		carspawned:SetPos( USCONVOY.Cfg.SpawnPos.vec )
		carspawned:SetAngles( USCONVOY.Cfg.SpawnPos.ang )
  ...
  timer.Simple( 60, function() ply:SetNWBool("Spawncooldown", false) end )
	...
	DarkRP.notify(ply, 1,3, "Your on cooldown, try again in a few moments! ") 
