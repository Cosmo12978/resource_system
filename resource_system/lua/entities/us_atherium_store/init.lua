...
if ply:getJobTable().category=="Civil Protection" then  
    if ply:HasWeapon("box_swep") then
        ...
        SetGlobalInt("USTICKETS",GetGlobalInt("USTICKETS") + 5)
    end
    ...
    if ply:HasWeapon("ath_box_swep") then
        ...
        ply:SetRunSpeed(400)
        ply:SetSlowWalkSpeed(150)
        SetGlobalInt("USATHERIUM",GetGlobalInt("USATHERIUM") + 1)
    end
...
function ENT:Think()
    SetGlobalInt("USATHERIUM",GetGlobalInt("USATHERIUM"))
    SetGlobalInt("USTICKETS",GetGlobalInt("USTICKETS") )
...
function ENT:OnTakeDamage(dmg)
   ...
   if(self.OurHealth <= 0) then 
    ...
    crate:Spawn()
    crate:Setatheriumamounts(GetGlobalInt("USATHERIUM") /2)
    SetGlobalInt("USATHERIUM", GetGlobalInt("USATHERIUM") /2)
    else
        SetGlobalInt("USATHERIUM",GetGlobalInt("USATHERIUM") /2 )
